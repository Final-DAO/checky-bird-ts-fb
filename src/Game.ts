namespace Floppy {
  export class Game {
    ...
    constructor(
      domElements: Floppy.Common.GameHtmlElements,
      options: Floppy.Common.GameOptions
    ) {
    ...
    protected set currentScore(newScore: number) {
      ...
      this.domElements.bigScore.replaceChildren(
        ...this.numberToImageElements(newScore, "big")
      );
    ...
    protected async die() {
      ...
      const wonMedal = this.medals.find(
        ([minimumScore]) => this.currentScore >= minimumScore
      );
